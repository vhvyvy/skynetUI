...
### Если дашборд не грузится (остаётся скелетон)

На Railway WebSocket через прокси иногда не успевает отдать стрим. **Временное решение — запуск без прокси:**

- **Start:** `python run_streamlit_port.py` (скрипт читает порт из переменной `PORT`; не используй `$PORT` в команде — на Railway он может не подставиться).
- В **Variables** задай только `APP_PASSWORD` (и БД, Notion и т.д.). Переменную `AUTH_PROXY` не ставь.
- Вход будет по форме прямо в Streamlit (сессия может сбрасываться при перезагрузке страницы, но дашборд будет открываться).
