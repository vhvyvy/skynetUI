...
1. **Streamlit без своего пароля**  
   В настройках приложения Streamlit **не задавай** `APP_PASSWORD` / `ADMIN_PASSWORD`, чтобы в самом Streamlit не было формы входа. Весь контроль доступа — только через прокси.
